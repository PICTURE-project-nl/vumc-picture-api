...
class SparsifiedTraining(AbstractTransform):
    def __init__(self,zeros_probability=0.2):
        self.zeros_probability = zeros_probability
        self.init_time = time.time()
    def __call__(self, **data_dict):
        imgs = data_dict['data']
        shape = imgs.shape
        if time.time()-self.init_time < 120:
            print(shape)
        for i_pt in range(shape[0]):
            for i in range(1,4):
                if np.random.uniform() < self.zeros_probability:
                    imgs[i_pt,i,::] = np.zeros(shape[2:])
                    if time.time()-self.init_time < 120:
                        print('zeros size'+str(shape[2:]))
        data_dict['data'] = imgs
        return data_dict
