...
class SegmentationNetwork(NeuralNetwork):
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
        ...
        if use_gaussian and num_tiles > 1:
            ...
            gaussian_importance_map = torch.from_numpy(gaussian_importance_map).to(device=self.get_device(),
                                                                                     non_blocking=True)
        ...
        if all_in_gpu:
            ...
            data = torch.from_numpy(data).to(device=self.get_device(), non_blocking=True)
    ...
    def _internal_maybe_mirror_and_pred_3D(self, x: Union[np.ndarray, torch.tensor], mirror_axes: tuple,
        ...
        result_torch = torch.zeros([1, self.num_classes] + list(x.shape[2:]),
                                   dtype=torch.float).to(self.get_device(), non_blocking=True)
    ...
    def _internal_predict_2D_2Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
        ...
        if use_gaussian and num_tiles > 1:
            ...
            gaussian_importance_map = torch.from_numpy(gaussian_importance_map).to(self.get_device(),
                                                                                     non_blocking=True)
        else:
            gaussian_importance_map = None
        ...
        if all_in_gpu:
            ...
            if verbose: print("moving data to GPU")
            data = torch.from_numpy(data).to(self.get_device(), non_blocking=True)
